...
namespace SimpleTrader.Domain.Tests.Services.TransactionServices
{
    ...
    public class SellStockServiceTests
    {
        ...
        public void SellStock_WithInsufficientShares_ThrowsInsufficientSharesException()
        {
            ...
            double actualAccountShares = exception.AccountShares;
            double actualRequiredShares = exception.RequiredShares;

            Assert.AreEqual(expectedSymbol, actualSymbol);
            Assert.AreEqual(expectedAccountShares, actualAccountShares);
            Assert.AreEqual(expectedRequiredShares, actualRequiredShares);
        }

        [Test]
        public void SellStock_WithInvalidSymbol_ThrowsInvalidSymbolExceptionForSymbol()
        {
            string expectedInvalidSymbol = "bad_and_never_exist_symbol";
            ...
            InvalidSymbolException exception = Assert.ThrowsAsync<InvalidSymbolException>(() => _sellStockService.SellStock(seller, expectedInvalidSymbol, 10));
        ...
        [Test]
        public async Task SellStock_WithSufficientShares_ReturnAccountWithNewTransaction()
        {
            int expectedTransactionCount = 2;
            Account seller = CreateAccount(It.IsAny<string>(), 10);

            seller = await _sellStockService.SellStock(seller, It.IsAny<string>(), 5);
            int actualTransactionCount = seller.AssertTransactions.Count;

            Assert.AreEqual(expectedTransactionCount, actualTransactionCount);
        }

        [Test]
        public async Task SellStock_WithSufficientShares_ReturnAccountWithNewBalance()
        {
